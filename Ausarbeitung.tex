...
Bei Betätigung des \textit{Analyse (Andre)} -Button werden die Daten nach dem Algorithmus von Andre ausgewertet. Pro Tag der Eingabedaten werden eine bestimmte Anzahl an Werten berechnet. Diese Werte werden in \textit{finaldata.csv} gespeichert. (siehe Abb.\ref{andre}) 
...
In der Funktion \textit{finaldata2csv} werden die berechneten Daten in der Datei \textit{finaldata.csv} abgespeichert.
...
\begin{tikzpicture} 
   \path[mindmap,concept color=orange,text=black]
    ...
    child[concept color=orange] {
      ...
      child { node[concept] {Analyse\\(Andre)} }
      child [concept color=blue, text = black]{ node[concept] {Change\\Epoch} }
      child { node[concept] {Wearing\\Time}
            [clockwise from=270]
    ...
    child[concept color=red] { node[concept] (rtc){RawTo\\Counts/\\METs}}
    }; ; 
...
\section{Unannehmlichkeiten bei der Implementierung}

Die Implementierung in \textsc{Java} erfolgte über den Texteditor \textsc{Eclipse}.\newline

Da von dem \textsc{Java} Programm auf \textsc{Matlab}-Funktionen zugegriffen wird, müssen die \textsc{Matlab}-Dateien bei jeder Änderungen neu als \textsc{Java} Klasse \textit{gebuildet} werden. Da es sich um viel \textsc{Matlab}-Code handelt, beansprucht dies sehr viel Zeit. Wenn der Entwickler nun eine Idee für eine Programmierung hat, sich aber derer nicht sicher ist, kostet ihm es viel Zeit, seine Erkenntnis zu testen.\newline

Zudem trat bei der Implementierung häufig der Fehler \textit{too many input arguments} auf. Durch visuelles Überpfrüfen der entsprechenden Funktion und den Aufruf mit seinen Übergabeparametern, konnten keine Fehler gefunden werden. Es handelte sich auch nicht um einen Fehler. Oftmals reichte es völlig aus entweder \textsc{Eclipse} und \textsc{Matlab} zu schließen oder erneut zu \textit{builden}.\newline

/TODO: sollen wir so etwas bringen? Wenn ja: Erweiterung
...
\footnotetext{mögliche Downloadseite: \textit{http://www.mathworks.de/products/compiler/}} und die \textsc{Matlab} \textbf{Signal Processing Toolboxe\footnotemark}
...
Zudem gibt es für jeden Sensor Besonderheiten, die beachtet werden müssen. Bei den beiden Sensoren Shimmer und Somnowatch muss die Frequenz 
sowohl zu Beginn eingestellt werden als auch nach dem Wechsel zu den Rohdaten.\newline

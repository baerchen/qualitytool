...
In diesem Schritt des Beispielablauf werden nun die Counts aus den Rohdaten berechnet. Natürlich besteht die Möglichkeit zuerst die Rohdaten zu \textit{plotten} (PlotAllData + Cut-Button) und zu beschneiden. Allerdings ist es für große Daten sinnvoll zuerst die Counts zu berechnen. Mit diesen dauert die Berechnung des ersten \textit{Plots} nicht so lange und die Daten können auf die selbe Art beschnitten werden. In den folgenden Abschnitten wird beschrieben wie der Benutzer danach über den \textit{Data type} zurück zu \textit{raw data measurement} wechseln kann um nur noch den \textit{Plot} für die ausgewählten Rohdaten durchzuführen.\newline
...
Zur Erstellung der Grafik für den ersten \textit{Plot} wird der \textit{$PlotAllData + Cut$}-Button gedrückt (siehe Kapitel \ref{Plot/Cut}). Für die Beispieldaten sieht dieser wie folgt aus:
...
In dem nächsten Schritt kann nun die Tragezeit nach van Hees berechnet werden. Da sich diese ausschließlich mit Rohdaten berechnen lässt, werden für die Auswertung die gerade zugeschnittenen Daten verwendet.

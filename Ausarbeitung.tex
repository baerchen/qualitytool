...
Auch bei dem \textit{Plotten} wird zunächst von der \textit{KoraSteps.class} auf \textsc{Matlab} zugegriffen. Von \textit{$step2\_plotFile$} wird je nach \textit{measurement} \textit{$cutFirst1\_Raw$} bzw. \textit{$cutFirst1\_new$} aufgerufen. In dieser Funktion werden die Achsen des \textit{Plots} beschriftet, Informationen zum Sensor der Graphik, sowie die Legende zu den Graphfarben festgesetzt. Unterhalb des Graphen wird ein Fenster eingerichtet. Dieses ermöglicht es dem Benutzer, den wesentlichen Teil des Graphen zu bestimmen. Nach dieser Festlegung werden die Daten zugeschnitten.\newline
